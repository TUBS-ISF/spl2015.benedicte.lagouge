...
import core.sound.specific.MarioSoundManager10512Hz;
...
public class Mario extends CollidableObject{
	...
	private int score;
	public Mario(MarioSoundManager22050Hz soundManager,String ss) {
			...
			if(collision && !(creature instanceof Score)) {
				...
				if(creature instanceof Coin) {
					...
					map.creaturesToAdd().add(new RedShell(Math.round(creature.getX()), 
							Math.round(creature.getY()+13), map, soundManager, true,this.sounds));
