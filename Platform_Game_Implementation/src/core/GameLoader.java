...
import objects.creatures.Goomba;
import objects.creatures.Platform;
import objects.creatures.RedKoopa;
import objects.creatures.RedShell;
import objects.tiles.QuestionBlock;
import objects.tiles.RotatingBlock;
		...
		for (int y=0; y < height; y++) {
			...
			for (int x=0; x < line.length(); x++) {
				...
				if (ch == 'G') {
					...
					QuestionBlock q = new QuestionBlock(pixelX, pixelY, newMap, soundManager, true, false,this.sounds);
					newMap.setTile(x, y, q);
					newMap.animatedTiles().add(q);
				} else if (ch == 'W') {
					QuestionBlock q = new QuestionBlock(pixelX, pixelY, newMap, soundManager, false, true,this.sounds);
					newMap.setTile(x, y, q);
					newMap.animatedTiles().add(q);
				} else if (ch == 'S') {
					newMap.creatures().add(new RedShell(pixelX, pixelY, newMap, soundManager, true,this.sounds));
				} else if(ch == 'C') {
